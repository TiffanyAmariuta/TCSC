...
#	row.pval = grep( "pval" , rownames(cv.performance) )	
	...
	if ( !is.na(opt$force_model) ) {
		best.mod = 1 #1 = which( colnames(wgt.matrix) == opt$force_model )
#		if ( length(1) == 0 ) {
#			cat( "WARNING : --force_model" , 1 ,"does not exist for", unlist(wgtlist[w,]) , "\n")
#			cur.FAIL = TRUE
#		}	
	} else {
		# get the most significant model
		best.mod = 1 #which.min(apply(cv.performance[row.pval,,drop=F],2,min,na.rm=T))
	}
	if ( length(1) == 0 ) {
	...
	if ( sum(wgt.matrix[, 1] != 0) == 0 ) {
		cat( "WARNING : " , unlist(wgtlist[w,]) , "blup had", length(cur.Z) , "overlapping SNPs, but none with non-zero expression weights, try more SNPS or a different model\n")
		cur.FAIL = TRUE
	}

	# if this is a top1 model, clear out all the other weights
	#if ( substr( (colnames(cv.performance))[ 1 ],1,4) == "top1" ) wgt.matrix[ -which.max(wgt.matrix[,1]^2)  , 1] = 0
	...
	if ( length(cur.Z) == 0 ) {
		...
		if ( sum(cur.miss) != 0 ) {
			if ( sum(!cur.miss) == 0 ) {
				...
				if ( sum(is.na(all.r2pred)) != 0 ) {
					...
					cur.FAIL = TRUE
				} else if ( mean( all.r2pred[ wgt.matrix[,1] != 0 ] ) < opt$min_r2pred ) {
					cat( "WARNING : " , unlist(wgtlist[w,]) , "had mean GWAS Z-score imputation r2 of" , mean( all.r2pred[ wgt.matrix[,1] != 0 ] ) , "at expression weight SNPs, skipping this gene.\n")
					cur.FAIL = TRUE
				}
			}
		}
		...
		if ( !cur.FAIL ) {
			...
			cur.twasz = wgt.matrix[,1] %*% cur.Z
			cur.twasr2pred = wgt.matrix[,1] %*% cur.LD %*% wgt.matrix[,1]
			...
			if ( cur.twasr2pred > 0 ) {
				...
				if ( !is.na(opt$perm) && opt$perm > 0 && cur.twas^2 > permz^2 ) {
					...
					perm.pval = NA
					for ( i in 1:opt$perm ) {
						perm.wgt = wgt.matrix[ sample( nrow(wgt.matrix) ) , 1 ]
	...
	out.tbl$MODEL[w] = "meta" #colnames( cv.performance )[ 1 ]
	out.tbl$MODELCV.R2[w] = "noMetaR2" #paste(format(cv.performance[row.rsq,1],digits=2,trim=T),collapse=',')
	out.tbl$MODELCV.PV[w] = "noMetaP" #paste(format(cv.performance[row.pval,1],digits=2,trim=T),collapse=',')
	...
	if ( cur.FAIL || sum(eqtlmod) == 0 ) {
		...
		out.tbl$EQTL.ID[w] = rownames(wgt.matrix)[topeqtl] #names( topeqtl ) (editted August 7 TA-B) fails when wgt.matrix is of nrow = 1
		out.tbl$EQTL.R2[w] = "nor2" # cv.performance[1,eqtlmod]
	...
	if ( !cur.FAIL ) {
		out.tbl$NWGT[w] = sum( wgt.matrix[,1] != 0 )
