...
X <- as.matrix(fread("coregulation_scores/CoregulationMatrix_allGTEx_030422.txt.gz", header = F))
...
gene_annot <- fread(paste0("weights/allEUR_tissues/v8_allEUR_",tissue,"_blup.pos"), header = T)
...
save(list=c("wkeepX","X","transcripts","starts","chrs","tissueassign"), file = "analysis/InputCoreg_BrainSpecific.RData")
