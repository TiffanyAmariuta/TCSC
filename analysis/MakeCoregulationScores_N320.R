...
library(data.table)
ciswindow <- 2e6
gene_ref <- fread("analysis/AllGenesRef_in_v8320smalltoo_analysis.txt.gz") #might not have all genes including genes from small tissues (missing 2) from tissue 10
...
if(i %in% small_tissues){load(paste0("predicted_expression/Nall/designmat_",tissues[i],"_v8_allEUR_double.RData"))}else{
load(paste0("predicted_expression/N320/designmat_",tissues[i],"_v8_320EUR_double.RData"))}
print(dim(df1))
df1_focal <- df1
rm("df1")
if(i %in% small_tissues){transcript_key <- fread(paste0("weights/heritablegenes/Nall/TranscriptsIn",tissues[i],"Model.txt"), header = F)$V1
keeptranscripts <- fread(paste0("weights/heritablegenes/Nall/TranscriptsIn",tissues[i],"Model_keep.txt"), header = F)$V1
}else{
transcript_key <- fread(paste0("weights/heritablegenes/N320/TranscriptsIn",tissues[i],"Model.txt"), header = F)$V1
keeptranscripts <- fread(paste0("weights/heritablegenes/N320/TranscriptsIn",tissues[i],"Model_keep.txt"), header = F)$V1
}
...
if(j %in% small_tissues){load(paste0("predicted_expression/Nall/designmat_",tissues[j],"_v8_allEUR_double.RData"))}else{
load(paste0("predicted_expression/N320/designmat_",tissues[j],"_v8_320EUR_double.RData"))}
...
keeptranscripts <- fread(paste0("weights/heritablegenes/Nall/TranscriptsIn",tissues[j],"Model_keep.txt"), header = F)$V1}else{
...
write.table(X, file = paste0("coregulation_scores/CoregulationMatrix_320orless.txt"), row.names = F, col.names = F, sep = "\t", quote = F)
