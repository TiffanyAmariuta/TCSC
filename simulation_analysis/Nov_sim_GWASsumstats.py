...
def sim_eqtl(g, ti, sim, Z_qtl, nqtl, b_qtls, eqtl_h2):
    ...
    tempdir = "/n/groups/price/tiffany/subpheno/Simulations/twas_sim/Sims_biascorrec_minhsqthresh/temps_ggcoreg/temp_gene"+str(g)+"/temp_gene"+str(g)+"_tissue"+str(ti)+"_sim"+str(sim)+"_nqtl"+str(nqtl)
    b = b_qtls[:,0]
    ...
    if r2_train>0 and r2>0:
        h2g,hsq_se,hsq_p = get_hsq(Z_qtl,gexpr,tempdir,"/n/groups/price/tiffany/subpheno/fusion_twas-master/gcta_nr_robust")
    else:
        h2g=0
        hsq_se=10
        hsq_p=1
...
filename = "TCSC/simulation_analysis/Simulated_GWAS_Cohort_%s.txt.gz" % ngwas
z_gwas = np.array(pd.read_csv(filename, sep = "\t", header = None))
print("simulated full GWAS cohort")

#only gene-trait direct effects via snps. 
if ve != 0:
    filename = "TCSC/simulation_analysis/ComplexTraits/%s_Nov/ComplexTraits_Nov_Group1_vgene%s_sim%s.txt.gz" % (ngwas,ve,sim)
    trait1 = np.array(pd.read_csv(filename, sep = "\t", header = None))[:,0] #first trait only, not second

#also snp-trait direct effects 
if snp != 0:
    filename = "TCSC/simulation_analysis/ComplexTraits/%s_Nov/ComplexTraits_Nov_Group1_vgene%s_vsnp%s_sim%s.txt.gz" % (ngwas,ve,snp,sim)
    trait2 = np.array(pd.read_csv(filename, sep = "\t", header = None))[:,0]
...
if ve != 0:
    gwas1 = regress(z_gwas, trait1) #z-gwas has snps on columns people on row 
if snp != 0:
    gwas2 = regress(z_gwas, trait2)

if ve != 0:
    stats = np.zeros([z_gwas.shape[1],3])
    stats[:,0] = gwas1.pval.values
    stats[:,1] = gwas1.beta.values
    stats[:,2] = gwas1.se.values
    filename = "TCSC/simulation_analysis/GWASsumstats/sumstats_%s_vgene%s_sim%s.txt.gz" % (ngwas,ve,sim)
    stats = pd.DataFrame(stats)
    stats.round(4).to_csv(filename, sep="\t", index=False,header = False)

if snp != 0:
    stats = np.zeros([z_gwas.shape[1],3])
    stats[:,0] = gwas2.pval.values
    stats[:,1] = gwas2.beta.values
    stats[:,2] = gwas2.se.values
    filename = "TCSC/simulation_analysis/GWASsumstats/sumstats_%s_vgene%s_vsnp%s_sim%s.txt.gz" % (ngwas,ve,snp,sim)
    stats = pd.DataFrame(stats)
    stats.round(4).to_csv(filename, sep="\t", index=False,header = False)
