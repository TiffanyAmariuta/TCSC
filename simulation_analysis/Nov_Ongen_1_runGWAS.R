...
#double check 
snpscheck <- fread("1KG_HM3_chr1.bim", header = F)
...
for (sim in 1:sims){
    ...
    w <- which(p < 5e-8)
    if(length(w) > 0){ 
        filename <- paste0("Ongen_files/gwas_snps/Nov_sim",sim,"_ve",ve,".txt")
        ...
        filename <- paste0("Ongen_files/gwas_null_snps/Nov_sim",sim,"_ve",ve,".txt")
